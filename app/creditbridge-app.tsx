...
export default function CreditBridgeApp() {
  ...
  function runPipeline() {
    ...
    let step = 0;
    const timer = window.setInterval(() => { step += 1; setActiveStep(step); if (step >= 5) { window.clearInterval(timer); setPipeline("review"); notify("Analysis complete — one decision requires your review"); } }, 430);
  }
  function decide(decision: "approve" | "escalate") {
  ...
  function onFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (names.length) notify(`${names.length} document${names.length > 1 ? "s" : ""} added to this case`);
  }
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      <header className="topbar"><div><p className="eyebrow">TRANSFER CREDIT OPERATIONS</p><h1>{view === "case" ? "Decision workspace" : view === "audit" ? "Evidence audit trail" : "Advisor command center"}</h1></div><div className="top-actions"><button className="icon-button" aria-label="Notifications">●<span /></button><button className="secondary" onClick={() => fileRef.current?.click()}>＋ Add documents</button><input ref={fileRef} type="file" multiple hidden onChange={onFiles} accept=".pdf,.doc,.docx,.txt" /><button className="primary" onClick={runPipeline}>{pipeline === "running" ? "Analyzing…" : "Run agents"}<span>→</span></button></div></header>

      {view === "queue" && <Overview onOpen={() => setView("case")} />}
      {view === "audit" && <Audit />}
      {view === "case" && <>
        <section className="case-strip"><div><button className="back" onClick={() => setView("queue")}>← All cases</button><div className="case-title"><span className="case-id">CB-2026-0148</span><h2>IVC CS 38 → UCLA Computer Science</h2><span className={`status-pill status-${pipeline}`}>{statusLabel}</span></div></div><div className="case-meta"><span>Student <strong>S. Wang</strong></span><span>Received <strong>Aug 21, 2026</strong></span><span>SLA <strong className="good">18h remaining</strong></span></div></section>
        {uploaded.length > 0 && <div className="upload-banner"><strong>New evidence ready</strong><span>{uploaded.join(" · ")}</span><button onClick={runPipeline}>Analyze now</button></div>}
        <section className="metrics"><Metric label="Recommended match" value={selectedMatch} detail="UCLA Computer Science" /><Metric label="Evidence confidence" value="87%" detail="14 of 16 outcomes aligned" accent /><Metric label="Estimated time saved" value="2.4h" detail="Per evaluation case" /><Metric label="Decision risk" value="Medium" detail="Lab-depth ambiguity" /></section>
        ...
        <section className="content-grid"><div className="left-stack">
          ...
          <article className="panel evidence-panel"><div className="panel-head"><div><p className="section-kicker">EVIDENCE GRAPH</p><h3>Outcome-level comparison</h3></div><button className="text-button" onClick={() => notify("All 14 evidence citations verified against source text")}>View all 14 citations ↗</button></div><div className="course-headings"><div><span>SOURCE COURSE</span><strong>IVC CS 38</strong><small>Java Programming</small></div><div className="match-score"><span>87%</span><small>semantic + policy score</small></div><div><span>CANDIDATE MATCH</span><strong>UCLA {selectedMatch}</strong><small>Computer Organization</small></div></div><div className="outcomes">{[["Object-oriented design", "Direct", "96%"], ["Data structures & algorithms", "Direct", "93%"], ["Memory & machine representation", "Partial", "71%"], ["Assembly programming laboratory", "Missing depth", "42%"]].map(([a,b,c],i) => <div className="outcome" key={a}><span className="outcome-num">0{i+1}</span><strong>{a}</strong><span className={b.includes("Missing") ? "warn" : b === "Partial" ? "partial" : "match"}>{b}</span><b>{c}</b></div>)}</div><div className="source-proof"><span className="proof-icon">“</span><div><strong>Source-grounded finding</strong><p>IVC CS 38 covers Java memory models and references, but the submitted syllabus does not demonstrate assembly-language lab work comparable to CS 33 weeks 2–5.</p><small>IVC_CS38_Syllabus.pdf · pages 4–6 · SHA-256 verified</small></div></div></article>
        </div>
...
function Audit() {
  return <div className="audit-page"><section className="audit-summary"><div><span>CASE</span><strong>CB-2026-0148</strong><small>Every claim traceable to source</small></div><div><span>EVENTS</span><strong>5</strong><small>0 mutations detected</small></div><div><span>POLICY</span><strong>v2.4</strong><small>Effective Aug 01, 2026</small></div><div><span>INTEGRITY</span><strong className="good">Verified</strong><small>Hash chain intact</small></div></section><article className="panel audit-table"><div className="panel-head"><div><p className="section-kicker">PROVENANCE LEDGER</p><h3>Case events</h3></div><button className="secondary" onClick={() => window.print()}>Export receipt</button></div><div className="audit-table-head"><span>Time</span><span>Actor</span><span>Action</span><span>Control</span><span>Event ID</span></div>{auditRows.map(row => <div className="audit-table-row" key={row[4]}><span>{row[0]}</span><strong>{row[1]}</strong><span>{row[2]}</span><em className={row[3] === "Review" ? "review" : "verified"}>{row[3]}</em><code>{row[4]}</code></div>)}</article><article className="integrity panel"><div className="integrity-mark">✓</div><div><h3>Evidence chain verified</h3><p>All agent inputs, tool calls, policy evaluations, and human decisions are linked by tamper-evident receipts. No event has been removed or modified.</p></div><code>root: 4b87…c91a</code></article></div>;
}
